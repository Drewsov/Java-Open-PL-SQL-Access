...
// OWA.java
//
// $Id: OWA.java,v 1.81 2007/01/01 10:08:16 Drew Exp $
//
//----------------------------------------------------------------------
...
class OWA extends Processor_PSP
{
...
throws JOPAException
{
  ...
  String stmt = shell.getCode("sendEnvOWA", "{A}", ARRAY_TYPE);
  try {
...
private Vector describeProc(String sProc)
throws JOPAException
{
...
private int getOverloadCount(String sProc)
throws JOPAException
{
  ...
  try
  {
    if(m_getOvcCS == null)
    {
      m_getOvcCS = (OracleCallableStatement)m_conn.prepareCall(shell.getCode("getOverloadCount"));
    }
    m_getOvcCS.registerOutParameter(2, OracleTypes.NUMBER);
    m_getOvcCS.setString(1, sProc);
    m_getOvcCS.execute();
    iOver = m_getOvcCS.getInt(2);
    if (m_getOvcCS.wasNull()) iOver = 1;
  }
  catch (SQLException e) {
    try 
    { m_conn.clearWarnings(); } 
    ...
    { }
    shell.log(5, "getOverloadCount exception: "+e.getMessage());
    iOver = 1;
  }
  return iOver;
}
